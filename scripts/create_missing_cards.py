...
FROM MEDICORE.MARTS.MART_KPI_MARGE m
JOIN MEDICORE.MARTS.DIM_PRODUIT p ON m.PRODUIT_SK = p.PRODUIT_SK
...
SELECT
    ...
    COUNT(*) AS nb_lignes
FROM MEDICORE.MARTS.MART_KPI_MARGE
...
FROM MEDICORE.MARTS.MART_KPI_RUPTURES r
JOIN MEDICORE.MARTS.DIM_PRODUIT p ON r.PRODUIT_SK = p.PRODUIT_SK
GROUP BY 1 ORDER BY 2 DESC
LIMIT 10""",
        C["achats"], "bar",
    )
...
FROM MEDICORE.MARTS.MART_KPI_ECOULEMENT e
JOIN MEDICORE.MARTS.DIM_FOURNISSEUR f ON e.FOURNISSEUR_SK = f.FOURNISSEUR_SK
GROUP BY 1 ORDER BY 2 DESC
LIMIT 15""",
        C["achats"], "bar",
    )
...
FROM MEDICORE.MARTS.MART_KPI_GENERIQUE
GROUP BY 1""",
        C["qualite"], "bar",
    )
...
FROM MEDICORE.MARTS.FACT_VENTES v
JOIN MEDICORE.MARTS.DIM_PRODUIT p ON v.PRODUIT_SK = p.PRODUIT_SK
GROUP BY 1 ORDER BY 2 DESC
LIMIT 20""",
        C["detail"], "bar",
    )
...
FROM MEDICORE.MARTS.FACT_VENTES
GROUP BY 1 ORDER BY 2 DESC""",
        C["detail"], "bar",
    )
