...
- **CDC dual path** — Kafka temps reel + bulk quotidien
- **Micro-batch** — 500 events ou timeout 30s
- **Incremental merge** — dbt merge sur cle primaire composite
- **Star schema** — Dimensions (surrogate keys) + Faits (FK + mesures)
- **PII masking** — MD5 dans macros dbt staging
- **DLQ** — Dead Letter Queue pour events malformes
- **Health checks** — Docker + Snowflake connectivite
...
- **CI/CD** : Non configure (orchestration via batch_loop.sh)
