...
- JAMAIS `pd.read_sql(chunksize=)` avec `mysql.connector` — bufferise l'integalite du resultset en memoire avant de chunker
- Utiliser `cursor(buffered=False)` + `fetchmany(chunk_size)` pour streaming server-side reel
...
- Chunk size 500K rows pour rester sous 7.5 GiB de memoire conteneur
